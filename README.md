...
- 予測モデルの高度化は単純な精度向上（LightGBM等への全面移行）では採用できない。裁定スコアは適正価格モデルの残差そのものなので、表現力の高いモデルほど「客観スペック」だけでなく「業者の値付けの癖」「売り急ぎ等の個別事情」まで学習してしまい、検出したい市場の歪み自体を吸収して残差を消してしまうリスクがある。方向性としては、DAGに基づく変数統制と解釈可能性を保ったまま、客観スペック側の機能形の柔軟性のみ（age等の非線形項）を上げる形が妥当（詳細は`docs/phase2_summary.md` 10節）。
