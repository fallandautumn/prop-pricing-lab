...
v1（単純な重回帰、R²=0.86）はブラックボックスで、各変数がなぜ家賃に効いているのかを説明できなかった。v2ではこれを解消するために因果推論を導入したが、当初は「因果推論を組み込めば説明可能性も予測精度も両方上がるはずだ」という素朴な理解だった。

この理解が不正確だったことは、実装を進める中で段階的に露呈した。Phase 1の時点では、階数の建物固定効果モデル（+0.57%/階）と適正価格ヘドニックモデル（+1.02%/階）で係数の大きさに乖離が出ており、この時点では「未観測のbuilding間交絡が混入している」という観察に留まっていた（`docs/phase1_summary.md` 3節）。Phase 2では、新たに作ったLLM特徴量（`brand_score`・`location_score`）を因果効果モデルと適正価格モデルの両方に同じ形で追加してみたところ、`total_floors`の非有意化など係数の不安定な挙動が発生し（`docs/phase2_summary.md` 4.2節）、さらに`location_score`単体を深掘りしたところ`area_group`とのVIF=12.04という深刻な多重共線性が判明した（同9節）。

これらを通じて、**予測（相関を総動員してE[Y|X]を最も正確に当てるタスク）と、因果推定（交絡を意図的に除去して特定属性の純粋な効果を測るタスク）は、そもそも目的関数が異なる**という結論に最終的に至った。建物固定効果（`C(building_id)`）は「階数の純粋な効果」を測るには最適だが、固定効果自体は未知の建物には使えないため予測には使えない。逆に、相関をフル活用する予測モデルの係数を「因果効果」として読むと、交絡込みの見せかけの効果を拾ってしまう。

この結論に基づき、`matching.py`/`cem.py`（係数解釈専用）と`scoring.py`（`divergence_rate`計算専用）を明確に別モジュールとして再定義した。プロジェクトの目的自体も、「割安物件検出エンジン」という単一目的の説明から、「市場相場予測（裁定検出）」と「属性プレミアム分析（潜在価格推定）」を分離・統合した分析基盤という定義に改めている。

①全部屋スクレイピング（v1は建物につき1部屋のみだった）、②分析ロジックとDBアクセスの分離、も並行して行った反省点。
